...
import (
	...
	"github.com/xoxo/crm-x/util/errors"
...
func New(directoryPath string) (*Database, error) {
	...
	if err != nil {
		logger.AppendError("unable to initialize the badger-based session database: %v", err)
		return nil, err
	}
...
func (db *Database) Load(sid string) (storeDB sessions.RemoteStore) {
	...
	if err != nil {
		logger.AppendError("error while trying to get the serialized session(%s) from the remote store: %v", sid, err)
		return
	}

	storeDB, err = sessions.DecodeRemoteStore(b) // decode the whole value, as a remote store
	if err != nil {
		logger.AppendError("error while trying to load from the remote store: %v", err)
	}
...
func (db *Database) sync(p sessions.SyncPayload) {
	...
	if p.Action == sessions.ActionDestroy {
		if err := db.destroy(bsid); err != nil {
			logger.AppendError("error while destroying a session(%s) from badger: %v",
				p.SessionID, err)
		}
		return
	}
	...
	if err != nil {
		logger.AppendError("error while serializing the remote store: %v", err)
	}
	...
	if err != nil {
		txn.Discard()
		logger.AppendError("error while trying to save the session(%s) to the database: %v", p.SessionID, err)
		return
	}
	if err := txn.Commit(nil); err != nil { // Commit will call the Discard automatically.
		logger.AppendError("error while committing the session(%s) changes to the database: %v", p.SessionID, err)
	}
}
...
func closeDB(db *Database) error {
	...
	if err != nil {
		logger.AppendWarn("closing the badger connection: %v", err)
	}
	return err
}
